...
```bash
./scripts/check.sh           # doctor only (always run)
./scripts/check.sh --new-run # doctor + create artifacts/runs/run-<ts>/evidence/
```
...
1. **Doctor First**: At the start of a session, or after any tool failure, run `./scripts/check.sh` (or `control_doctor` from the agent). Don't wait for errors to discover missing deps — `check.sh` exits non-zero on missing assets.
2. **RUN_ID discipline**: Always create a run dir with `check.sh --new-run` for any multi-step task. Never overwrite previous evidence.
