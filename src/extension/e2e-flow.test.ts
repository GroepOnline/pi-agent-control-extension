...
import { routeControlTask } from "../core/routing/routing.ts";
import { browserControlGuidance } from "../drivers/browser/browser.ts";
import { buildUsageReport } from "../core/utils.ts";
import { capture, parseCaptureArgs, formatCaptureMarkdown, routeToDriver } from "./capture.ts";
import { validateEvidence } from "../core/types/evidence.ts";
...
describe("E2E: Capture → Evidence → Validation Flow", () => {
  ...
  it("rejects invalid evidence input during validation", () => {
    ...
    expect(validation.errors.some((e: string) => e.includes("evidenceId"))).toBe(true);
    expect(validation.errors.some((e: string) => e.includes("format"))).toBe(true);
  });
