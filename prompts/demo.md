...
---
Runtime note: install this package with `agy install .`; then invoke this as `/demo` and load `agy-agent-control`.


Load skills: **agy-agent-control**.
...
Use the **agy-agent-control** routing tables. Do all three lookups:
...
**Delegation:** For before/after comparisons, capture both branches **in parallel** using worker subagents with `run_in_background=true`. Construct the exact `tctl` commands for each worker (see the delegation section in the agy-agent-control skill). Wait for both to complete before proceeding to compose.
